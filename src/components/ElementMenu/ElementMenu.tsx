...
import { useCanvasEditor, useFabric } from '@/hooks/context';
...
function ElementMenu() {
  ...
  useEffect(() => {
    console.log('useEffect', canvasEditor)
  ...
  const addText = (option) => {
    const text = new fabric.IText('everything_is_fine', {
      ...defaultPosition,
      ...option,
      fontSize: 80,
      id: uuid(),
  ...
  const addTextBox = (option) => {
    const text = new fabric.Textbox('everything_goes_well', {
      ...defaultPosition,
      ...option,
...
const drawingLineModeSwitch = (isArrow: boolean) => {
  const _isDrawingLineMode = !isDrawingLineMode;
  const _isArrow = isArrow;
  setIsDrawingLineMode(_isDrawingLineMode)
  setIsArrow(_isArrow)
  canvasEditor.setMode(_isDrawingLineMode);
  canvasEditor.setArrow(_isArrow);

  // this.canvasEditor.setMode(this.isDrawingLineMode);
  // this.canvasEditor.setArrow(isArrow);
  canvasEditor.canvas.forEachObject((obj) => {
    if (obj.id !== 'workspace') {
      obj.selectable = !isDrawingLineMode;
      obj.evented = !isDrawingLineMode;
    }
  });
};
  ...
  return (
    <div>
      <Divider>基础元素</Divider>
      <div className="tool-box">
        <span onClick={() => addText()} draggable={true} onDragEnd={() =>onDragend('text')}>
          <svg
            t="1650875455324"
            className="icon"
        ...
        <span onClick={() => addTextBox()} draggable={true} onDragEnd={() =>onDragend('textBox')}>
          <svg
            t="1650854954008"
            className="icon"
            viewBox="0 0 1024 1024"
        ...
        <span onClick={() => addRect()} draggable={true} onDragEnd={() =>onDragend('rect')}>
          <svg
            t="1650855811131"
            className="icon"
            viewBox="0 0 1024 1024"
        ...
        <span onClick={() => addCircle()} draggable={true} onDragEnd={() =>onDragend('circle')}>
          <svg
            t="1650855860236"
            className="icon"
            viewBox="0 0 1024 1024"
        ...
        <span onClick={() => addTriangle()}  draggable={true} onDragEnd={() =>onDragend('triangle')}>
          <svg
            t="1650874633978"
            className="icon"
            viewBox="0 0 1024 1024"
        ...
        <span onClick={() => addPolygon()}  draggable={true} onDragEnd={() =>onDragend('polygon')}>
          <svg
            t="1650874633978"
            className="icon"
            viewBox="0 0 1024 1024"
      ...
      <Divider>绘制元素</Divider>
      <div className="tool-box">
        <span
          onClick={() => drawingLineModeSwitch(false)}
          className={isDrawingLineMode && !isArrow ? 'selected' : ''}
        >
          <svg
            t="1673022047861"
            class="icon"
        ...
        <span
          onClick={() => drawingLineModeSwitch(true)}
          className={isDrawingLineMode && isArrow ? 'selected' : ''}
        >
          <svg
            t="1673026778912"
